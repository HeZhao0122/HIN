...
def parse_args():
    ...
    parser.add_argument('-dataset', required=True, type=str, help='Targeting dataset.', 
                        choices=['DBLP','PubMed','Freebase','DBLP2','ACM','ACM2'])
    parser.add_argument('-model', required=True, type=str, help='Targeting model.', 
                        choices=['metapath2vec-ESim','Node2Vec','HIN2Vec','MAGNN','HGT','CKD','GraphSNN_GCN', 'SLAPS-GNN'])
        ...
        for i, line in enumerate(emb_file):
            if i == 0:
                ...
                try:
                    ...
                    index = line[0]
                    emb = line[1:]
                    emb_dict[index] = np.array(emb).astype(np.float32)
...
def check(args):
    ...
    if args.attributed=='True':
        if args.model not in ['R-GCN', 'HAN', 'MAGNN', 'HGT','CKD','HDGI','NSHE','GraphSNN_GCN', 'SLAPS-GNN', 'Node2Vec']:
            ...
            return False
        if args.dataset not in ['DBLP','PubMed','Freebase','DBLP2','ACM','ACM2']:
