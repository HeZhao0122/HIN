...
class data_loader:
    def __init__(self, path, symetric, edge_types=[]):
        self.path = path
        self.splited = False
        self.symetric = symetric
        self.nodes = self.load_nodes()
        self.links = self.load_links('link.dat')
        self.test_types = list(self.links['data'].keys()) if edge_types == [] else edge_types
    ...
    def load_types(self, name):
        ...
        with open(os.path.join(self.path, name), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.strip().split('\t')
                node_id, node_type = int(th[0]), int(th[1]),
    ...
    def load_links(self, name):
        ...
        with open(os.path.join(self.path, name), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                if name == 'link.dat':
                    h_id, h_type, t_id, t_type, r_id = int(th[0]), int(th[1]), int(th[2]), int(th[3]), int(th[4])
                    ...
                    links['total'] += 1
        # 非对称的adjM
        if self.symetric==False:
            org_count = len(links['count'])
            org_rids = [r_id for r_id in links['data']]
            for r_id in org_rids:
                h_type, t_type = links['meta'][r_id]
                new_r_id = r_id + org_count
                links['meta'][new_r_id] = (t_type, h_type)
                links['count'][new_r_id] = links['count'][r_id]
                for (h_id, t_id, weight) in links['data'][r_id]:
                    links['data'][new_r_id].append((t_id, h_id, weight))
                    links['total'] += 1
