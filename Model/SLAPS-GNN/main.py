...
from data_loader import load_data, load_hin_data
from model import GCN, GCN_C, GCN_DAE
from utils import accuracy, get_random_mask, get_random_mask_ogb, nearest_neighbors, normalize, output
...
class Experiment:
    ...
    def get_loss_learnable_adj(self, model, mask, features, labels, Adj, idx=None):
        logits, embs = model(features, Adj)
        # embs = logits
        if idx != None:
            logits = logits[idx]
            logp = F.log_softmax(logits, 1)
            loss = F.nll_loss(logp, labels[mask], reduction='mean')
            accu = accuracy(logp, labels[mask])
        else:
            logp = F.log_softmax(logits, 1)
            loss = F.nll_loss(logp[mask], labels[mask], reduction='mean')
            accu = accuracy(logp[mask], labels[mask])
        return loss, accu, embs
    ...
    def train_end_to_end(self, args):
        # features, nfeats, labels, nclasses, train_mask, val_mask, test_mask = load_data(args)
        features, nfeats, labels, nclasses, test_mask, val_mask, train_mask, test_idx, val_idx, train_idx = load_hin_data(args)
        ...
        for trial in range(args.ntrials):
            ...
            for epoch in range(1, args.epochs_adj + 1):
                ...
                if epoch < args.epochs_adj // args.epoch_d:
                    ...
                    loss1, Adj = self.get_loss_masked_features(model1, features, mask, ogb, args.noise, args.loss)
                    loss2, accu, _ = self.get_loss_learnable_adj(model2, train_mask, features, labels, Adj, train_idx)
                ...
                if epoch >= args.epochs_adj // args.epoch_d and epoch % 2 == 0:
                    with torch.no_grad():
                        model1.eval()
                        model2.eval()

                        val_loss, val_accu, val_embs = self.get_loss_learnable_adj(model2, val_mask, features, labels, Adj, val_idx)
                        if val_accu > best_val_accu:
                            best_val_accu = val_accu
                            print("Val Loss {:.4f}, Val Accuracy {:.4f}".format(val_loss, val_accu))
                            output(args, val_embs)
                            test_loss_, test_accu_, _ = self.get_loss_learnable_adj(model2, test_mask, features, labels, Adj, test_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset', type=str, default='cora', help='See choices',
                        choices=['cora', 'citeseer', 'pubmed', 'ogbn-arxiv', 'ogbn-proteins', 'ACM2', 'ACM', 'DBLP', 'DBLP2'])
    ...
    parser.add_argument('-output', type=str, default="emb.dat")
