...
def parse_args():
    ...
    parser.add_argument('-model', required=True, type=str, help='Targeting model.',
                        choices=['metapath2vec-ESim', 'HIN2Vec', 'R-GCN', 'HAN', 'MAGNN', 'HGT',
                                 'TransE', 'DistMult', 'ComplEx', 'ConvE','CKD','HDGI','OpenNe','GraphSNN_GCN', 'Node2Vec'])
...
def check(args):
    ...
    if args.supervised == 'True':
        if args.model not in ['R-GCN', 'HAN', 'MAGNN', 'HGT', 'GraphSNN_GCN', "SLAPS-GNN"]:
...
def main():
    ...
    elif args.model=='GraphSNN_GCN':GraphSNN_covert(args.dataset, args.attributed)
    elif args.model == 'Node2Vec':node2vec(args.dataset)
