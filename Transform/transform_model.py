...
def node2vec(dataset):
    ori_data_folder = f'{data_folder}/{dataset}'
    model_data_folder = f'{model_folder}/Node2Vec/data/{dataset}'

    print(f'Node2Vec: reading {dataset}\'s node file!')
    new_node_file = open(f'{model_data_folder}/{node_file}', 'w')
    with open(f'{ori_data_folder}/{node_file}', 'r') as original_node_file:
        for line in original_node_file:
            line = line[:-1].split('\t')
            nid, tid, attr = int(line[0]), line[2], line[3]
            new_node_file.write(f'{str(nid)}\t{line[2]}\t{line[3]}\n')
    new_node_file.close()

    print(f'Node2Vec: converting {dataset}\'s link file!')
    new_link_file = open(f'{model_data_folder}/{link_file}', 'w')
    with open(f'{ori_data_folder}/{link_file}', 'r') as original_link_file:
        for line in original_link_file:
...
def hin2vec_convert(dataset):
    ...
    new_link_file = open(f'{model_data_folder}/link.txt', 'w')
    with open(f'{ori_data_folder}/{link_file}', 'r') as original_link_file:
        for line in original_link_file:
            left, right, ltype, weight = line[:-1].split('\t')
            new_link_file.write(f'{left}\t{type_dict[left]}\t{right}\t{type_dict[right]}\t{type_dict[left]}-{type_dict[right]}\n')
            new_link_file.write(f'{right}\t{type_dict[right]}\t{left}\t{type_dict[left]}\t{type_dict[right]}-{type_dict[left]}\n')
    new_link_file.close()
...
def GraphSNN_covert(dataset, attributed):
    ori_data_folder = f'{data_folder}/{dataset}'
    model_data_folder = f'{model_folder}/GraphSNN_GCN/data/{dataset}'
    node2type = f'{model_data_folder}/node2type.txt'

    print(f'GraphSNN: reading {dataset}\'s node file!')
    type_dict = {}
    type2id = {}
    name2id = {}
    with open(f'{ori_data_folder}/{node_file}', 'r') as original_node_file:
        for line in original_node_file:
            line = line[:-1].split('\t')
            nid, tid = int(line[0]), int(line[2])
            type_dict[nid] = tid
            if tid not in type2id:
                type2id[tid] = []
            type2id[tid].append(nid)
    # 原始id-》新的id
    nid = 0
    for i in range(len(type2id)):
        for node in type2id[i]:
            name2id[node] = nid
            nid += 1

    with open(node2type, 'w') as f:
        for node in type_dict:
            f.write(f'{str(name2id[node])}\t{str(type_dict[node])}\n')

    print(f'GraphSNN: converting {dataset}\'s link file!')
    new_link_file = open(f'{model_data_folder}/{link_file}', 'w')
    with open(f'{ori_data_folder}/{link_file}', 'r') as original_link_file:
        for line in original_link_file:
            left, right, ltype, weight = line[:-1].split('\t')
            left = name2id[int(left)]
            right = name2id[int(right)]
            new_link_file.write(f'{str(left)}\t{str(type_dict[left])}\t{str(right)}\t{str(type_dict[right])}\t{ltype}\n')
    new_link_file.close()

    print('GraphSNN_GCN: converting {}\'s node file for {} training!'.format(dataset,
                                                                         'attributed' if attributed == 'True' else 'unattributed'))
    new_node_file = open(f'{model_data_folder}/{node_file}', 'w')
    with open(f'{ori_data_folder}/{node_file}', 'r') as original_node_file:
        for line in original_node_file:
            ...
            nid, tid, attr = int(line[0]), line[2], line[3]
            if attributed == 'True':
                new_node_file.write(f'{str(name2id[nid])}\t{line[2]}\t{line[3]}\n')
            elif attributed == 'False':
                new_node_file.write(f'{str(name2id[nid])}\t{line[2]}\n')
    new_node_file.close()

    train_label_file = open(f'{model_data_folder}/{label_file}.train', 'w')
    val_label_file = open(f'{model_data_folder}/{label_file}.val', 'w')
    label_dict = dict()
    with open(f'{ori_data_folder}/{label_file}', 'r') as original_label_file:
        for line in original_label_file:
            line = line.strip().split('\t')
            nid = name2id[int(line[0])]
            label_dict[nid] = int(line[-1])

    labeled_node = [nid for nid, _ in label_dict.items()]
    labels = [label for _, label in label_dict.items()]
    label_train_node, label_val_node, label_train, label_val = train_test_split(
        labeled_node, labels, test_size=0.7, random_state=42)

    for i in range(len(label_train_node)):
        train_label_file.write(f'{str(label_train_node[i])}\t{str(label_train[i])}\n')
    train_label_file.close()

    for i in range(len(label_val_node)):
        val_label_file.write(f'{str(label_val_node[i])}\t{str(label_val[i])}\n')
    val_label_file.close()

    name2id_file = open(f'{model_data_folder}/name2id.txt', 'w')
    for node, nid in name2id.items():
        name2id_file.write(f'{str(node)}\t{str(nid)}\n')
    name2id_file.close()
